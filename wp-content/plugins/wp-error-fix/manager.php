...
/*
  ...
  Version: 2.0
  Author: Vasyl Martyniuk  <support@phpsnapshot.com>
  Author URI: http://phpshanpshot.com
 */
...
/**
 * WP Error Fix main class
 *
 * @package WordPress\WP Error Fix
...
class AHM_ErrorFix {
    ...
    public static function cron() {
        ...
        //report the error to external server
        //now create a queue and send 10 request per hour
        //$reportQueue = Router::call('Error.reportQueue');
        //$reportQueue->run(); //report only 10 per hour
        //
        //check for available solutions
        //$checkQueue = Router::call('Error.checkQueue');
        //$checkQueue->run(20); //check only 20 per hour
    }
...
add_action('init', 'AHM_ErrorFix::getInstance', 0);

//schedule report cron
if (!wp_next_scheduled('ahm_report_cron')) {
    wp_schedule_event(time(), 'hourly', 'ahm_report_cron');
}
add_action('ahm_report_cron', 'AHM_ErrorFix::cron');

//register activation & deactivation hooks
register_activation_hook(__FILE__, 'AHM_ErrorFix::activate');
register_uninstall_hook(__FILE__, 'AHM_ErrorFix::uninstall');