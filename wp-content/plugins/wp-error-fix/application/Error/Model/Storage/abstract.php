...
abstract class AHM_Error_Model_Storage_Abstract implements Iterator {
    ...
    public function add($time, $level, $msg, $file, $line, $checksum, $n = 1) {
        ...
        if (isset($this->_storage[$hash])) {
            ...
            $this->_storage[$hash] = $this->findSolution($hash, array(
                'level' => $level,
    ...
    /**
     * Find if this error was already reported or resolved
     * 
     * @param string $seach
     * 
     * @return string
     ...
     * @access protected
     */
    protected function findSolution($search, $report){
        $report['status'] = AHM_ERROR_STATUS_NEW;
        foreach (Router::call('Error.errors') as $storage) {
            foreach ($storage as $hash => $info) {
                if (($hash == $search) 
                        && ($info['status'] != AHM_ERROR_STATUS_FAILED)){
                    $report['status'] = $info['status'];
                    if (isset($info['patch'])){
                        $report['patch'] = $info['patch'];
                        $report['price'] = $info['price'];
                    }
                }
            }
        }
