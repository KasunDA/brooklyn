        ...
        jQuery.ajax(ajaxurl, {
            ...
            success: function(response) {
                if (response.status === 'success') {
                    _this.drawGraph(name, response.data);
                    _this.cache[name] = response.data;
                    //show empty message if total is zero
                    if (response.warning === 1) {
                        jQuery('.empty-error-log span').html(
                                'Content Folder is not writable'
                                );
                        display_note = true;
                        ...
                        var display_note = false;
                    }
                    if (display_note) {
                        jQuery('.empty-error-log').css('display', 'table');
                    } else {
                        jQuery('.empty-error-log').css('display', 'none');
                    }
                }
            },
            error: function() {
                ...
                jQuery.plot(jQuery('#graph-pie'), data, {
                    ...
                    legend: {
                        labelFormatter: function(label, series) {
                            ...
                            var str = label + '  (';
                            if (typeof series.percent !== 'undefined'){
                                str += series.percent.toFixed(2);;
                            } else{
                                str += '?';
                            }
                            str += '%)';
