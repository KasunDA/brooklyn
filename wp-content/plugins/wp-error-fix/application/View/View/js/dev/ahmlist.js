...
ahmList.prototype.initList = function() {
    ...
    this.list = jQuery('#list').dataTable({
        ...
        fnRowCallback: function(nRow, aData) { //format data
            //decorate Last Time (Occured)
            var total = (aData[1][1] <= 100 ? aData[1][1] : '100+');
            jQuery('td:eq(1)', nRow).html(aData[1][0] + ' (' + total + ')');
            //decorate Status
            if (typeof ahmLocal.statuses[aData[2]] !== 'undefined'){
                var tooltip = ahmLocal.statuses[aData[2]];
            } else {
                tooltip = '';
            }
            jQuery('td:eq(2)', nRow).html(jQuery('<span/>', {
                title: tooltip + aData[6],
                class: 'ahm-error-status'
            }).html(aData[2]));
            _this.parent.addTooltip(jQuery('.ahm-error-status', nRow));
        },
